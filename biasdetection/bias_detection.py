...
data = pd.read_csv(settings.DATASET_PATH, na_values=["Unknown", " "],) #dtype=settings.DATA_DTYPE)
data_orig = data.copy()
...
for each_column in (cat_columns):
    ...
    for each_sub_cat in (sub_cat):
        # print(each_column)
        # print(each_sub_cat)
        # print(len(data[(data[each_column] == each_sub_cat)]))
